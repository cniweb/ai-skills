...
description: >
  Subagent skill for validating GitHub issue changes via
  targeted and broader quality checks, then reporting
...
Validate GitHub issue changes with prioritized checks
and return verifiable results.

Validiere GitHub-Issue-Aenderungen mit priorisierten
Checks und liefere nachvollziehbare Ergebnisse.
...
1. Select checks by priority.
   ...
   - Then run broader checks (lint, type-check, build,
...
1. Fix within validation scope when appropriate.

   - Resolve introduced errors directly related to
     changed scope.
...
1. Fixes Applied During Validation
...
- Do not claim full validation when key checks were
  skipped.
- If full suite cannot run, run closest subset and
  document limits.
- Keep remediation scoped to errors introduced by the
  issue work.
